...
/* Scheduling. */
#define TIME_SLICE 4   
#define TIME_SLICE0 4
#define TIME_SLICE1 5
#define TIME_SLICE2 6
#define TIME_SLICE3 7         /* # of timer ticks to give each thread. */
...
thread_tick (void) 
{
  ...
  /* Enforce preemption. */
  thread_ticks++;
  if((thread_ticks >= TIME_SLICE0) && t->priority == 0)
    intr_yield_on_return();
  if((thread_ticks >= TIME_SLICE1) && t->priority == 1)
    intr_yield_on_return();
  if((thread_ticks >= TIME_SLICE2) && t->priority == 2)
    intr_yield_on_return();
  if((thread_ticks >= TIME_SLICE3) && t->priority == 3)
    intr_yield_on_return();
  thread_aging();
}
...
thread_aging(void){
  ...
  for(int i = t->priority + 1; i < 4 ;i++){
    ...
    for (e = list_begin(&ready_list[i]);e != list_end(&ready_list[i]); e = e->next){
      ...
      not_current_FQ = list_entry(e, struct thread, elem);
      
      if(!is_thread(not_current_FQ)) break;
      if(not_current_FQ->status == THREAD_BLOCKED) break;
      //age를 올리고 20이 넘었으면 높은 큐로 올려
      not_current_FQ->age++;
      if(not_current_FQ->age >= 20){
        if(not_current_FQ->priority!=0){
          not_current_FQ->priority--;
        }
        not_current_FQ->age = 0;//한단계 위로 올라갔으면 age 0으로 초기화 시켜야함.
        list_pop_front(&ready_list[not_current_FQ->priority+1]);
        list_push_back(&ready_list[not_current_FQ->priority], &not_current_FQ->elem);
      }
    }
  }
}
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread){
    if(cur->priority!=3){// 가장 낮은 priority가 아닐경우 priority를 1씩 올려서 우선순위를 낮춘다
      cur->priority += 1;
    }
